...
class hopfield(inputs):
 def __init__(self,real_structure,band_structure):
  ...
  self.LM=[LM_at+[[i[0],-i[1]] for i in LM_at if i[1]!=0] for LM_at in real_structure.LM] # we have to take into account also the m<0 , see charge.f from SRC_lapw5
 ...
 def calc_r_and_k_integrals(self):
  ...
  self.blm_integrals=[blm_all_integrals(self.ALMBLM[at],self.ENE_kweights,inputs.n_l,self.n_k,self.n_k_total) 
       for at in range(self.n_at) ]
...
#def Hopfield(at,nlm1,dr,LM,RMESH,Vtot,RADWF,ALMBLM):
  ...
  half_of_no_of_lm1_and_2=int(no_of_lm1_and_2/2)
  Plm,dPlm=generate_Plm(10)
  Ylm0=generate_Ylm_0(Plm)
  print (at,nlm1,LM[at])
  Hopfield=0.j
  HopfieldS=0.j #with SOC
...
#        continue 
  ...
  #       Cx+= -4*np.pi**2*l1*(l1-1)/(((2*l1-1)*(2*l1+1))**0.5)*spec_int(l1-1,m1,l3,m3,l4,m4)
       ...
       [B_and_C_r,A_r]=chosen_r_integrals(r_integrals[nlm1%half_of_no_of_lm1_and_2][l3][l4])
...
#            continue
           ...
           [B_and_C_rp,A_rp]=chosen_rp_integrals(r_integrals[nlm2%half_of_no_of_lm1_and_2][l5][l6])
...
#              -l2*(l2-1)/(((2*l2-1)*(2*l2+1))**0.5)*spec_int(l2-1,m2,l5,m5,l6,m6))   
           ...
           hopfield_contrib=Ax*A+B_and_C_x*B_and_C
           if m1!=0: hopfield_contrib=hopfield_contrib/(2.**0.5) 
           if m2!=0: hopfield_contrib=hopfield_contrib/(2.**0.5) 
           if m1%2==0 and m1<0: hopfield_contrib=-hopfield_contrib
           if m2%2==0 and m2<0: hopfield_contrib=-hopfield_contrib # see SRC_lapw5/charge.f, there is a sum (ylm+MINUS*yl-m)/sqrt2, however we are making integrals, so we cannot sum harmonics at the beginning
           Hopfield+=hopfield_contrib
           if l1==0 and l2==0 and (Ax!=0 or B_and_C_x!=0): print (l3,l4,l5,l6,Ax,np.dot(almblm_k,almblm_kp),np.dot(A_r,A_rp),np.dot(B_and_C_r,B_and_C_rp),Bx,Cx)
  print('finally: ',at,nlm1,round_complex(Hopfield,9))
  print('finally: ',at,nlm1,round_complex(HopfieldS,9))
  return [Hopfield,HopfieldS]
...
def blm_integral(l3,m3,l5,m5,ALMBLM_at, weight,n_k):
 results=np.array([0j,0j,0j,0j,0j,0j,0j,0j,0j])
 for k in range(n_k):
...
def blm_all_integrals(ALMBLM_at, weight,n_l,n_k,n_k_total):
  ...
  for m3 in range(-l3,l3+1):
   for l5 in range(n_l):
    for m5 in range(-l5,l5+1):
     blm_integrals[l3][l3+m3][l5][l5+m5]=(blm_integral(l3,m3,l5,m5,ALMBLM_at, weight, n_k)) /suma
...
r_int34[8],r_int34[8],r_int34[8],\
r_int34[8],r_int34[8],r_int34[8]])
 return [i[0] for i in B_and_C_and_A_r],[i[1] for i in B_and_C_and_A_r]
