...
for ni,i in enumerate(tmp):
 if 'TOTAL POTENTIAL IN INTERSTITIAL' in i:
  n_K_intersitial=int(tmp[ni+2].split()[0])
...
print('Read full k-grid...'),
h=open('DOS/DOS.outputkgen')
tmp=h.readlines()
recip_vectors=[]
for ni,i in enumerate(tmp):
 if 'G1        G2        G3' in i and len(recip_vectors)==0:
  recip_vectors=[ [float(m) for m in k.split()] for k in tmp[ni+1:ni+3] ]
 elif 'NO. OF MESH POINTS IN THE BRILLOUIN ZONE' in i:
  if n_k_total!=float(i.split('=')[-1[):
   raise ValueError('No of energy points != no of mesh generated by kgen')
 elif 'point    coordinates     relation' in i:
  tmp=tmp[ni+1:ni+1+n_k_total]
  break

ALLK=[ [float(m) for m in i.split()[1:4]] for i in tmp]
WHICHK=[int(i.split()[4]-1 for i in tmp]
